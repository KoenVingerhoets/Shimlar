...
function x2(lastmon,lastfight,opname,btlstr){
  top.lastmon=lastmon;top.lastfight=lastfight;
  top.opname=opname;top.battlestr=btlstr;
}
function x3(invs){
...
function xT(traders,titems,tprices,tids){
  top.trader=traders.split(",");
  top.titem=titems.split(",");
  top.tprice=tprices.split(",");
  top.tid=tids.split(",");  
}
function xB(zbname){
  top.zbname=zbname;top.zhunt=1;
}
function xD(ds,da,di){
  top.duelnames=ds.split(",");
  top.duelalign=da.split(",");
  top.duelid=di.split(",");
}
function xQ(num,mons,mlist,zone,exp,gold,qitem,qstat,qminlvl,qlvl,qlife){
  top.qmon[qx1]=mons;top.qmlist[qx1]=mlist;top.qzone[qx1]=zone;
  top.qexp[qx1]=exp;top.qgold[qx1]=gold;top.qitem[qx1]=qitem;top.qlife[qx1]=qlife;
  top.qstatus[qx1]=qstat;top.qnum[qx1]=num;top.qminlvl[qx1]=qminlvl;top.qlvl[qx1]=qlvl;qx1++;
}
...
function eX(errnum){
  switch(errnum){
  case(errnum==1||errnum==26||errnum==59):
...
function setIgn(namelist){
  top.setCookie("pIgnore",namelist);
}

function setNoRace(cVal){
  top.setCookie("noRaceChat",cVal);
}
